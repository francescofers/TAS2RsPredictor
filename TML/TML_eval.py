...
def eval_smiles(smiles, ground_truth):
    ...
    mol_fps = [Calc_fps(mol,1024,2) for mol in std_clean_smiles]
    fps_df = pd.DataFrame([np.frombuffer(fp.ToBitString().encode(), 'u1') - ord('0') for fp in mol_fps], index=std_clean_smiles, columns=[f'f_{i+1}' for i in range(1024)])
    fps_df = fps_df.astype(int)
    selected_fps = [x for x in selected_columns if 'f_' in x]
    fps_df = fps_df[selected_fps]
    """
    ...
    data = mord_fs_df
    """
    # Merging fps and mordred
    data = pd.concat([fps_df,mord_fs_df],axis=1)
    """
    ...
    """
    data_enc.columns = list(fps_df.columns.tolist() + mord_fs_df.columns.tolist() + hTAS2R)
    """
